  ...
  do {
    ...
    for (pred_iterator PI = pred_begin(BB); PI != pred_end(BB); ++PI) {
      ...
      if (Visited.insert(predBB).second)
        Worklist.push_back(WorkItem((predBB), (predBB)->end()));
		}
...
char MemoryIdempotenceAnalysis::ID = 1;
//char* taskAnalysisID = &TaskAnalysis::ID;
RegisterPass<MemoryIdempotenceAnalysis> X("ratchet-analysis", "Analysis for Ratchet");
